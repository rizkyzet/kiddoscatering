...
class Validation_model extends CI_Model
{
    ...
    public function my_password_check_siswa($password)
    {
        $siswa_passowrd = $this->db->get_where('siswa', ['nis' => $this->session->userdata('nis')])->row_array();

        if ($password) {
            if (password_verify($password, $siswa_passowrd['password'])) {
                return true;
            } else {
